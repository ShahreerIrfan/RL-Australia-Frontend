...
async function getRegionMap(cacheId: string) {
  ...
  if (
    ...
    if (!BACKEND_URL) {
      // No backend configured — use mock regions so the frontend still works
      console.warn("Middleware: MEDUSA_BACKEND_URL not set, using mock regions")
      regions = mockRegionsResponse.regions
    } else {
      // Fetch regions from Medusa. We can't use the JS client here because middleware is running on Edge and the client needs a Node environment.
      const result = await fetch(`${BACKEND_URL}/store/regions`, {
        headers: {
        ...
        console.warn("Middleware fetch failed, falling back to mock regions:", err.message)
